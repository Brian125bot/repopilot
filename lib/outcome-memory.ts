...
export function buildFailureBrief(
  ...
  return {
    ...
    auditedHeadSha: blueprint.auditedHeadSha ?? null,
    verdict,
    score,
    unmetIds,
    partialIds,
...
export function compileContinuationPrompt(input: {
  ...
  const lock = [
    ...
    `Audited head SHA: \`${blueprint.auditedHeadSha || 'MISSING — re-evaluate before remediation'}\`.`,
...
export function buildOutcomeRow(input: {
  ...
  return {
    ...
    auditedHeadSha: input.blueprint.auditedHeadSha ?? null,
